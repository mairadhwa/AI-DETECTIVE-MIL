...
(function () {
  ...
  /**
   * Draw the next question for a category. Returns the full quiz
   * question object plus category metadata and its 1-based index
   * within that category (mainly useful for debugging/analytics):
   *   { category, questionNumber, question, options[4], correctIndex, explanation }
   *
   * ===== INTERACTIVE QUIZ: question data now comes from
   * window.MIL.QUESTIONS (js/questions.js) instead of a static
   * card image. The shuffle-bag "no repeat until exhausted" deck
   * mechanic is unchanged — it just now deals an index into the
   * question array instead of an image filename. =====
   */
